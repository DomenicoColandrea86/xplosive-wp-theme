...
class CI_videos extends WP_Widget {
		...
		if ( ! empty( $title ) )
			...
			<div class="latest-item latest-video">
				...
				<div id="wplayer<?php echo $i; ?>-wrap" class="video-player">
					<div id="wplayer<?php echo $i; ?>">Loading the player ...</div>
	...
	function form($instance){
		...
		echo '<p><label>' . __('Videos Number:','ci_theme') . '</label><input id="' . $this->get_field_id('videos_no') . '" name="' . $this->get_field_name('videos_no') . '" type="text" value="' . $videos_no . '" class="widefat" /></p>';
	} // form
