...
class CI_galleries extends WP_Widget {
		...
		if ( ! empty( $title ) )
			...
			<div class="latest-item gallery-item">
				...
				<p class="album-info">
					...
					<span class="album-actions"><a href="<?php the_permalink(); ?>" class="action-btn view"><?php _e('View set','xplosive'); ?></a></span>
				</p>
			</div><!-- /gallery-item -->
			<?php
	...
	function form($instance){
		...
		echo '<p><label>' . __('Galleries Number:','ci_theme') . '</label><input id="' . $this->get_field_id('galleries_no') . '" name="' . $this->get_field_name('galleries_no') . '" type="text" value="' . $galleries_no . '" class="widefat" /></p>';
	} // form
