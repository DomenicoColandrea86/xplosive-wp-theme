	...
	if ($wp_query->max_num_pages > 1): ?>
		...
		>
			<?php if (function_exists('wp_pagenavi')): ?>
				<?php wp_pagenavi(); ?>
			<?php else: ?>
				<div <?php echo (empty($args['prev_link_class']) ? '' : 'class="'.$args['prev_link_class'].'"'); ?>><?php next_posts_link( '<span class="nav-prev-symbol nav-symbol">&laquo;</span> ' . __('Older posts', 'ci_theme') ); ?></div>
				<div <?php echo (empty($args['next_link_class']) ? '' : 'class="'.$args['next_link_class'].'"'); ?>><?php previous_posts_link( __('Newer posts', 'ci_theme') . ' <span class="nav-next-symbol nav-symbol">&raquo;</span>' ); ?></div>
			<?php endif; ?>
		</div>
	<?php endif;
}
